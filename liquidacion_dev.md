...
Lista todos los `LiquidacionMedico` del resumen.
...
**Precondiciones:**
- Al menos una liquidación del resumen debe estar `estado = "C"` (cerrada).
- Deben existir registros en `liquidacion_medico` (ejecutar `generar_liquidacion_medico` antes).
...
**Response 409** — sin liquidaciones cerradas o sin `liquidacion_medico`.
